...
export default function LiquidEther({
  ...
  useEffect(() => {
    ...
    class MouseClass {
      ...
      init(container) {
        this.container = container;
        // Attach to window so mouse/touch are tracked even when the canvas is behind UI (-z-10)
      ...
      setCoords(x, y) {
        ...
        // Track whether the pointer is over the container bounds (for auto driver behavior)
...
`;
    ...
    // Prime initial motion for immediate visual activity on load
    Mouse.setNormalized(0.15, 0.1);
  ...
  return (
    <div
      ref={mountRef}
      className={`w-full h-full relative overflow-hidden bg-black pointer-events-none touch-none ${className || ''}`}
      style={style}
    />
  );
}
LiquidEther.propTypes = {
  mouseForce: PropTypes.number,
  cursorSize: PropTypes.number,
  isViscous: PropTypes.bool,
  viscous: PropTypes.number,
  iterationsViscous: PropTypes.number,
  iterationsPoisson: PropTypes.number,
  dt: PropTypes.number,
  BFECC: PropTypes.bool,
  ...
  autoIntensity: PropTypes.number,
  takeoverDuration: PropTypes.number,
  autoResumeDelay: PropTypes.number,
  autoRampDuration: PropTypes.number,
};
